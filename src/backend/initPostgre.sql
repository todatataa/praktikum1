...
CREATE UNIQUE INDEX review_unique_client_event_idx
...
-- Reviews (samo klienti koji imaju rezervisan event mogu oceniti svog organizatora)
INSERT INTO review (client_id, organizator_id, event_id, rating, comment, review_date) VALUES
    ...
    ALTER TABLE review ADD COLUMN IF NOT EXISTS event_id INTEGER REFERENCES event(id_event);
    CREATE UNIQUE INDEX IF NOT EXISTS review_unique_client_event_idx ON review (client_id, event_id) WHERE event_id IS NOT NULL;
